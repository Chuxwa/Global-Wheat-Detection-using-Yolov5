...
class CAConv(nn.Module):
    # Coordinate attentive convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True,
        ...
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.ca = CoordAtt(c2, c2, reduction)
...
class CABottleneck(nn.Module):
    # Coordinate attentive bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = CAConv(c1, c_, 1, 1)
        self.cv2 = CAConv(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2
...
class C3SE(nn.Module):
    # CSP SEBottleneck with se-convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = SEConv(2 * c_, c2, 1)  # act=FReLU(c2)
...
class C3CA(nn.Module):
    # CSP CABottleneck with ca-convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = CAConv(2 * c_, c2, 1)  # act=FReLU(c2)
