...
def run(
    ...
    if save_csv:
        with open(str(save_dir / "predict.csv") , "w", newline='') as f_csv:
            headers = ["image_id", "PredictionString"]
            writer = csv.DictWriter(f_csv, headers)
            writer.writeheader()
            f_csv.close()
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_csv:
                        xywh = (
                            (xyxy2x1y1wh(torch.tensor(xyxy).view(1, 4)))
                            .view(-1)
                            .tolist()
                        )  # normalized xywh

                        with open(str(save_dir / "predict.csv") , "a", newline='') as f_csv:
                            headers = ["image_id", "PredictionString"]
                            writer = csv.DictWriter(f_csv, headers)
                            datas  = {'image_id':p.name.replace('.jpg',''),'PredictionString':" ".join(map(lambda x:str(x), xywh))}
